...
2. 左下角：位置 (xpos, ypos)，纹理坐标 (0,1)
3. 右下角：位置 (xpos+w, ypos)，纹理坐标 (1,1)
...
我们看到这个四边形分别对应字体文件纹理映射的四个顶点（0,0）（0,1）（1,1）（1,0），然后我们把这四个顶点映射到四边形的四个顶点上，就得到了这个四边形。这个(U,V)实际上就是纹理中的坐标。这跟FreeType定义的坐标系是相关。

![freetype_texture_coords](https://freetype.org/freetype2/docs/glyphs/bitmaps-1.svg)

看起来我们这里应该是反过来了，但是代码中再反过来，所以就对了。好吧，本来就是反的，怪不得开头字体是从上往下显示，我又倒了一遍。

这里面实际上有一堆约定俗称的玩意，要把这个看清楚，我估计得花点时间。
